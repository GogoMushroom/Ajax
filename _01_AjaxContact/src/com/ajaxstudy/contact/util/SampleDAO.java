...
/*연락처 리스트를 저장하고 잇는 클래스 
 * DB에 연결하지 않고 컬랙션  객체에 주소데이터 보관
...
public class SampleDAO {
	...
	static {
		...
		for(int i =0; i<names.length; i++) {
			nextNo++;//일련번호 1부터 담음
			String tel ="010-1111-22" +(i+10); // 번호 마지막 뒷자리는 10부터 1씩 증가
			String address= "서울특별시";
			Contact c = new Contact(nextNo, names[i], tel, address);
			//위에서 생성한 연락처 정보를 연락처 목록에 하나씩 담음 
			//contact.get(i)로 하나식 꺼낼 수 있고
			//contact.get(i)의 내용은 {1, 고수, 010-1111-2210, 서울특별시}형식으로 이루어져있다
			contacts.add(i,c); //i랑 c는 왜 i랑c를 쓰는거지,,? 
		}
	}
	//전체 연락처 목록 조회
	public static ContactList getContacts() {
		//static 블록에서 생성한 목록으로 된 ContactList 객체 생성
		//(현제 페이지0, 페이지 사이즈0, 목록사이즈 contact.size(), List<Contact> contacts)
	...
	public static ContactList getContacts(int pageno , int pagesize) {
		...
		//(현제 페이지 pageno, 페이지 사이즈 pagesize, 목록사이즈 contact.size(), List<Contact>, temps)
	...
	public static void deleteContact(long no) {
		...
		for (int i =0; i < contacts.size(); i++) {
			...
			// 연락처의 일렬번호가 전달받은 일련변호와 같을떼ㅐ 목록에서 제거 
			if(c.getNo()==no) {
				contacts.remove(i);
				count++;
				break;
	...
	//여러개의 연락처 한 번에 수정
	//수정할 연락처 목록 매개변수로 받음
	public static int updataBatch(ContactList contactList) {
		//수정될 연락처의 개수 
		int count = 0;
		//매개변수에서pageNo, pageSize, totalCount는 제외한 연락처 목록만 뽑음  
